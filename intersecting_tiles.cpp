...
: offset_x(x_min), max_tile_id(map_width), TILE_EXPIRY_LEEWAY(leeway),
                                            ...
                                            double y2, bool outer_ring)
{
    ...
    // proper pair. So we add both a minimum and a maximum value for them
    // everything is ok. Being a inner or outer segment and its direction
    // does not matter here. Most buildings will fall in this category.
...
{
    // sort bounds first
    for (stripe_t::iterator it = min_bounds.begin(); it != min_bounds.end(); ++it) {
        std::sort(it->begin(), it->end());
    }
    for (stripe_t::iterator it = max_bounds.begin(); it != max_bounds.end(); ++it) {
        std::sort(it->begin(), it->end());
    }

    // look for overlapping intervals and remove them
    for (size_t column = 0; column < std::min(min_bounds.size(), max_bounds.size()); ++column) {
        for (size_t entry = 1; entry < std::min(min_bounds.at(column).size(), max_bounds.at(column).size()); ++entry) {
            if (min_bounds.at(column).at(entry) <= max_bounds.at(column).at(entry - 1)) { // interval overlapping
                min_bounds.at(column).at(entry) = std::min(min_bounds.at(column).at(entry), min_bounds.at(column).at(entry - 1));
                max_bounds.at(column).at(entry) = std::max(max_bounds.at(column).at(entry), max_bounds.at(column).at(entry - 1));
                // invalidate previous element in both vectors
                min_bounds.at(column).at(entry - 1) = std::numeric_limits<uint32_t>::max();
                max_bounds.at(column).at(entry - 1) = std::numeric_limits<uint32_t>::max();
            }
        }
    }
...
{
    if (next_idx_min < min_bounds.at(current_x).size()) {
        uint32_t minimum = min_bounds.at(current_x).at(next_idx_min);
        ++next_idx_min;
        if (is_valid_bounds_entry(minimum)) {
            return minimum;
        }
    }
    throw std::runtime_error("No minimum found.");
}

uint32_t intersecting_tiles_t::get_next_maximum()
{
    if (next_idx_max < min_bounds.at(current_x).size()) {
        uint32_t maximum = max_bounds.at(current_x).at(next_idx_max);
        ++next_idx_max;
        if (is_valid_bounds_entry(maximum)) {
            return maximum;
        }
    }
    throw std::runtime_error("No maximum found.");
}
...
{
    return next_idx_min < min_bounds.at(current_x).size() && next_idx_max < max_bounds.at(current_x).size();
}
...
intersecting_tiles_t::get_next_pair()
{
    ...
    uint32_t maximum;
    try {
        minimum = get_next_minimum();
        maximum = get_next_maximum();
